...
class Lore(commands.Cog):
    ...
    async def lore(self, ctx: commands.Context, username: str):
        ...
        # Generate Lore
        prompt = (f"Write a funny Cobblemon adventure story starring the player '{username}'. The story should be lighthearted and...")
        ...
        # Extract text
        lore_text = lore.get("generated_text", "No response generated.") if isinstance(lore, dict) else str(lore)

        # Create profile embed message
        embed = discord.Embed(
            color = chat_handler.name_to_color(username)
        )
        embed.description = lore_text
